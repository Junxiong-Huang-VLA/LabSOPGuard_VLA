...
Worker E owns the documentation/eval handoff lane for the current LabEmbodied evidence-package pass. This runbook stays within `D:\LabEmbodied\docs\*` and the optional eval fixture space under `D:\LabEmbodied\tests\fixtures\*`. It does not modify `src\key_action_indexer`, LabSOPGuard backend/frontend code, PTZ code, camera monitor code, or any multi-camera monitor scope.

Project scope constraints still apply on 2026-05-14:

- LabEmbodied mainline is long dual-view experiment video evidence extraction, time alignment, physical evidence, vector indexing, retrieval, and evaluation.
...
- Experiment root: `D:\LabEmbodied\outputs\experiments\db64d9d2-16ce-4de0-8fb3-597066531c31`
- Key-action index root: `D:\LabEmbodied\outputs\experiments\db64d9d2-16ce-4de0-8fb3-597066531c31\key_action_index`
- Evidence package root: `D:\LabEmbodied\outputs\experiments\db64d9d2-16ce-4de0-8fb3-597066531c31\material_references`
- Evidence package manifest: `D:\LabEmbodied\outputs\experiments\db64d9d2-16ce-4de0-8fb3-597066531c31\material_references\evidence_package_manifest.json`
...
| RI-007 | Historical rerun stderr references PTZ/camera proxy registration. | `D:\LabEmbodied\output\key_action_rerun_3ccd_swapped.stderr.log` logs `/api/v1/cameras` and `/api/v1/ptz-tracker`. | LabSOPGuard backend registers routes at import time. | Out of scope for Worker E. | Do not expand this in LabEmbodied; only record it as a scope guard warning. |
...
| T1 Scope guard | Documentation explicitly preserves `AGENTS.md` boundaries and excludes PTZ, cloud PTZ, five-camera orchestration, `/api/v1/cameras`, wireless SDKs, and multi-monitor recording as LabEmbodied core work. |
...
Use this only after confirming no other worker is actively using the target experiment output directory. Prefer creating a new experiment/run or preserving logs under `D:\LabEmbodied\output\` instead of overwriting baseline evidence.
...
git status --short
Get-Content -LiteralPath 'D:\LabEmbodied\AGENTS.md'
Get-Content -LiteralPath 'D:\LabEmbodied\outputs\experiments\db64d9d2-16ce-4de0-8fb3-597066531c31\key_action_index\job_status.json' -Raw
```
...
```powershell
Set-Location -LiteralPath 'D:\LabEmbodied'
$env:PYTHONPATH = 'D:\LabEmbodied\src'
...
& 'C:\Users\Xx7\.conda\envs\LabSOPGuard\python.exe' -m key_action_indexer.cli evidence-package-validate `
  --package-root 'D:\LabEmbodied\outputs\experiments\db64d9d2-16ce-4de0-8fb3-597066531c31\material_references' `
  --strict

& 'C:\Users\Xx7\.conda\envs\LabSOPGuard\python.exe' -m key_action_indexer.cli evidence-package-eval `
  --package-root 'D:\LabEmbodied\outputs\experiments\db64d9d2-16ce-4de0-8fb3-597066531c31\material_references' `
  --queries 'D:\LabEmbodied\tests\fixtures\evidence_package_eval_queries_2026_05_14.json' `
  --limit 5
```
...
- Draft decisions: `D:\LabEmbodied\outputs\experiments\<experiment_id>\key_action_index\metadata\gold_query_decisions.json`
- Confirmed benchmark: `D:\LabEmbodied\outputs\experiments\<experiment_id>\key_action_index\metadata\gold_query_benchmark.json`
- Worker E fixtures: `D:\LabEmbodied\tests\fixtures\*.json`
...
The fixture `D:\LabEmbodied\tests\fixtures\evidence_package_eval_queries_2026_05_14.json` is a schema-compatible starter set for `evidence-package-eval`. It has two lanes:
...
```powershell
Set-Location -LiteralPath 'D:\LabEmbodied'
python -m json.tool 'D:\LabEmbodied\tests\fixtures\evidence_package_eval_queries_2026_05_14.json' | Out-Null
```
...
```powershell
Set-Location -LiteralPath 'D:\LabEmbodied\frontend'
npm run build
```
...
- `D:\LabEmbodied\docs\worker_e_2026-05-14_evidence_runbook.md`
- `D:\LabEmbodied\tests\fixtures\evidence_package_eval_queries_2026_05_14.json`

Validation performed on 2026-05-14:

- `python -m json.tool D:\LabEmbodied\tests\fixtures\evidence_package_eval_queries_2026_05_14.json`: passed.
- `evidence-package-eval` with `PYTHONPATH=D:\LabEmbodied\src`, current package root, and the new fixture: passed; `query_count=8`, `insufficient_count=1`, scored `intent/status/label/event_type` rates all `1.0`.
...
- `npm run build` in `D:\LabEmbodied\frontend`: passed.
