...
export default function Layout() {
  ...
  const isExperimentArea = location.pathname === '/experiments' || location.pathname === '/windows' || location.pathname.startsWith('/experiments/') || location.pathname.startsWith('/upload')
  const isWorkspaceMaterials = location.pathname.startsWith('/materials')
  const isVideoMemory = location.pathname.startsWith('/memory') || location.pathname.startsWith('/video-memory')
        ...
        <div className="mx-auto max-w-[1760px] px-5 sm:px-8">
          <div className="flex min-h-16 flex-col items-stretch justify-center gap-3 py-3 sm:flex-row sm:items-center sm:justify-between sm:gap-8 sm:py-0">
            ...
            <nav className="flex w-full shrink-0 flex-wrap justify-start gap-x-4 gap-y-1 sm:w-auto sm:justify-end sm:gap-x-5" aria-label="Primary">
              ...
              <Link to="/memory" className={linkClass(isVideoMemory)}>
                实验室日报
              </Link>
            </nav>
          </div>
