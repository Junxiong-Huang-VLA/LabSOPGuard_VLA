...
function fieldValue(item: MaterialSearchItem, key: string) {
  const record = item as Record<string, unknown>
  const payload = item.payload && typeof item.payload === 'object' ? item.payload as Record<string, unknown> : {}
  return record[key] ?? payload[key]
}

function isRealMaterial(item: MaterialSearchItem) {
  if (item.exists === false) return false
  if (fieldValue(item, 'source_real') === false) return false
  if (fieldValue(item, 'placeholder') === true) return false
  if (fieldValue(item, 'dry_run') === true || fieldValue(item, 'dry_run_placeholder') === true) return false
  const reason = String(fieldValue(item, 'missing_reason') || '').toLowerCase()
  const pathMarkers = [
    fieldValue(item, 'file_name'),
    fieldValue(item, 'stored_file'),
    fieldValue(item, 'preview_url'),
    fieldValue(item, 'frame_path'),
    fieldValue(item, 'clip_file_path'),
    fieldValue(item, 'clip_url'),
    fieldValue(item, 'display_name'),
  ].join(' ').toLowerCase()
  return !(
    reason.includes('placeholder') ||
    reason.includes('poster') ||
    reason.includes('synthetic') ||
    pathMarkers.includes('placeholder') ||
    pathMarkers.includes('poster') ||
    pathMarkers.includes('synthetic')
  )
}
...
export default function WorkspaceMaterials() {
  ...
  async function load(text: string) {
    ...
    try {
      ...
      setItems((published.items || []).filter(isRealMaterial))
