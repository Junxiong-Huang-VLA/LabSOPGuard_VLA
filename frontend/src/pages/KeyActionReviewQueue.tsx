...
type TypeFilter = 'all' | 'qa_warning' | 'evidence_semantic' | 'segment' | 'micro_segment'
  ...
  return (
    <div className="space-y-5">
      <PageHero
        ...
        description="把 QA warning、低置信 segment、候选 micro、证据语义和检索评测集中到一个可确认、可驳回、可导出的审核工作台。"
        actions={(
          <>
            {id && (
          ...
          <EvidenceCard className="p-4">
            <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_24rem]">
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  {(['all', 'qa_warning', 'evidence_semantic', 'segment', 'micro_segment'] as TypeFilter[]).map(type => (
                    <button
                      key={type}
                      type="button"
