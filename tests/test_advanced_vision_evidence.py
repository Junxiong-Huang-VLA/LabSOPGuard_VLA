...
def test_advanced_vision_evidence_tracks_motion_and_liquid_candidate(tmp_path: Path) -> None:
    ...
    assert movement["visual_confirmation_level"] == "trajectory_rejected_by_gate"
    assert movement["confirmation_level"] == "rejected"
    ...
    assert set(movement["reject_reasons"]) & {"low_identity_confidence", "too_few_points"}
    assert movement["metrics"]["normalized_displacement"] > 1.0
    assert liquid["requires_human_confirmation"] is True
    assert liquid["confirmation_level"] == "rejected"
    assert liquid["visual_confirmation_level"] == "liquid_flow_candidate_rejected_by_gate"
    ...
    assert container["visual_confirmation_level"] == "container_open_close_confirmed_rejected_by_gate"
    assert container["status"] == "rejected"
...
def test_advanced_vision_evidence_confirms_liquid_level_and_container_with_inventory(tmp_path: Path) -> None:
    ...
    assert liquid_flow["visual_confirmation_level"] == "liquid_flow_confirmed_rejected_by_gate"
    assert liquid_flow["requires_human_confirmation"] is True
    assert liquid_flow["status"] == "rejected"
    ...
    assert liquid_level["status"] == "confirmed"
...
def test_advanced_vision_evidence_flags_same_class_identity_risk(tmp_path: Path) -> None:
    ...
    assert movement["metrics"]["track_source"] == "bbox_temporal_association"
    assert movement["metrics"]["identity_confidence"] < 0.75
