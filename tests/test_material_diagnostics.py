...
def test_material_diagnostics_exposes_candidate_pipeline_counts(tmp_path: Path):
    _setup_project_root(tmp_path)
    client = TestClient(backend_main.app)
    experiment_id = "candidate_diag_exp"
    exp_dir = backend_main._experiment_output_dir(experiment_id)
    exp_dir.mkdir(parents=True)
    (exp_dir / "experiment.json").write_text(json.dumps({"experiment_id": experiment_id}), encoding="utf-8")
    (exp_dir / "published_materials.json").write_text(
        json.dumps({"schema_version": "published_materials.v1", "experiment_id": experiment_id, "total": 0, "items": []}),
        encoding="utf-8",
    )
    ...
    rows = [
        {
            ...
            "candidate_group_id": "group_a",
            "candidate_status": "pending",
            "asset_kind": "keyframe",
            "yolo_annotation_rendered": False,
            "rerender_error": "no_hand_target_interaction_evidence:paper",
            "manual_correction": {"source": "frontend"},
            "objects": ["gloved_hand", "paper"],
            "vlm_semantics": {"status": "aligned"},
            "yolo_recheck": {
                "status": "failed",
                "diagnostics": {"invalid_reason_counts": {"missing_object_bbox": 2}},
            },
        },
        {
            "candidate_id": "candidate_b",
            "candidate_group_id": "group_b",
            "candidate_status": "approved",
            "asset_kind": "keyframe",
            "yolo_annotation_rendered": False,
            "rerender_error": "no_target_box",
            "detected_objects": ["beaker"],
            "vlm_semantics": {"status": "error"},
            "yolo_recheck": {"status": "passed"},
        },
        {
            "candidate_id": "candidate_c",
            "candidate_group_id": "group_c",
            "candidate_status": "deferred",
            "asset_kind": "clip",
            "yolo_annotation_rendered": True,
            "manual_corrected": True,
            "confirmed_objects": ["gloved_hand", "pipette", "tube"],
            "vlm_semantics": {"status": "disabled"},
            "yolo_recheck": {"status": "segment_level_review_required"},
        },
    ]
    (candidate_root / backend_main._MATERIAL_CANDIDATE_INDEX_FILENAME).write_text(
    ...
    assert response.status_code == 200
    candidate_diagnostics = response.json()["candidate_diagnostics"]
    assert candidate_diagnostics["candidate_file_total"] == 3
    assert candidate_diagnostics["candidate_group_total"] == 3
    assert candidate_diagnostics["yolo_annotation_rendered_false_count"] == 2
    assert candidate_diagnostics["no_target_box_count"] == 2
    assert candidate_diagnostics["rerender_error_family_counts"] == {
        "no_hand_target_interaction_evidence": 1,
        "no_target_box": 1,
    }
    assert candidate_diagnostics["vlm_status_counts"] == {"aligned": 1, "error": 1, "disabled": 1}
    assert candidate_diagnostics["manual_correction_count"] == 2
    assert candidate_diagnostics["multi_object_candidate_count"] == 2
    assert candidate_diagnostics["object_count_distribution"] == {"2": 1, "1": 1, "3": 1}
