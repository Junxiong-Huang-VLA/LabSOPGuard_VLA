...
from labsopguard.soak_test import make_dry_run_config, preflight_soak_test_sources
...
def test_capture_source_preflight_works_with_synthetic_sources():
    report = preflight_soak_test_sources(make_dry_run_config(duration_sec=1.0), read_attempts=1)

    assert report["status"] == "passed"
    assert report["camera_count"] == 2
    assert all(item["frame_read"] for item in report["results"])
