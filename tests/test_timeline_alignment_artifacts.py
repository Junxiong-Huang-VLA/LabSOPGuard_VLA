...
def test_run_artifacts_preserve_full_multimodal_semantic_sync_payload(tmp_path: Path):
    ...
    semantic_sync = {
        ...
        "run_id": "run_semantic_full",
        "status": "calibrated",
        "reference_stream": {"camera_id": "bottom_view", "view_type": "first_person"},
        "semantic_events": [
            {
                "event_id": "sem_evt_0001",
                "event_type": "liquid_transfer",
                "matched_streams": [{"camera_id": "bottom_view"}, {"camera_id": "top_view"}],
            }
        ],
        "sync_anchors": [
            {
                "camera_id": "top_view",
                "reference_camera_id": "bottom_view",
                "local_time_sec": 7.5,
    ...
    third_person = next(stream for stream in alignment["streams"] if stream["camera_id"] == "top_view")
