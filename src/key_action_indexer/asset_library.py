...
def _dry_run_placeholder(path: Path) -> bool:
    try:
        with path.open("rb") as handle:
            return handle.read(32).upper().startswith(b"DRY RUN")
    except OSError:
        return False
...
def _asset_marker_is_non_real(*values: Any) -> bool:
    text = " ".join(str(value or "").lower() for value in values)
    return any(marker in text for marker in ("placeholder", "poster", "synthetic", "dry_run", "dry-run"))
...
def _make_asset_row(
    *,
    ...
    placeholder = bool(dry_run or _asset_marker_is_non_real(path_text, source_type, role, payload.get("source_type")))
    source_real = bool(exists and not placeholder)
    row = {
        "asset_id": _asset_id(
        ...
        "missing_reason": None if source_real or asset_type == "text_asset" else _as_dict(quality).get("status") or "non_real_or_missing_asset",
        "source_type": source_type,
        "source_id": source_id_text,
        "segment_id": str(segment_id) if segment_id else None,
        "micro_segment_id": str(micro_segment_id) if micro_segment_id else None,
