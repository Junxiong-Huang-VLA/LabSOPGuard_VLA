...
from .frame_time_map import capture_sec_to_video_sec, video_sec_to_capture_sec
...
def _preview_clip_kwargs() -> dict[str, Any]:
    if not _env_truthy("KEY_ACTION_PREVIEW_CLIP_REENCODE", True):
        return {}
    return {
        "copy_first": False,
        ...
        "crf": _env_int("KEY_ACTION_PREVIEW_CLIP_CRF", 30),
        "preset": os.environ.get("KEY_ACTION_PREVIEW_CLIP_PRESET"),
        "max_width": _env_int("KEY_ACTION_PREVIEW_CLIP_MAX_WIDTH", 960),
        "include_audio": _env_truthy("KEY_ACTION_PREVIEW_CLIP_INCLUDE_AUDIO", False),
        "faststart": True,
    }


def _fast_locate_only() -> bool:
    return _env_truthy("KEY_ACTION_FAST_LOCATE_ONLY", False) or _env_truthy(
        "KEY_ACTION_DEFER_SEGMENT_ASSETS",
        False,
    )


def _fast_locate_include_first_person() -> bool:
    default = _env_truthy("KEY_ACTION_FAST_LOCATE_FINE_SCAN_BOTH_VIEWS", False) or _env_truthy(
        "KEY_ACTION_YOLO_SCAN_BOTH_VIEWS",
        False,
    )
    return _env_truthy("KEY_ACTION_FAST_LOCATE_INCLUDE_FIRST_PERSON", default)


def _fast_locate_segment_interactions_enabled() -> bool:
    return _env_truthy("KEY_ACTION_FAST_LOCATE_SEGMENT_INTERACTIONS", False)


def _global_time_to_video_sec(source: VideoSource, global_time: str | datetime) -> float:
...
def _video_sec_to_global_time(source: VideoSource, video_sec: float) -> str:
    capture_sec = max(0.0, video_sec_to_capture_sec(source, video_sec, use_frame_time_map="auto"))
    return local_sec_to_global_time(source, capture_sec).isoformat()
...
def ffmpeg_supports_encoder(encoder: str, ffmpeg: str | None = None) -> bool:
    encoder_name = str(encoder or "").strip()
    ffmpeg_exe = ffmpeg or _find_ffmpeg_exe()
    if not encoder_name or not ffmpeg_exe:
        return False
    cache_key = (ffmpeg_exe, encoder_name)
    if cache_key in _FFMPEG_ENCODER_SUPPORT_CACHE:
        return _FFMPEG_ENCODER_SUPPORT_CACHE[cache_key]
    try:
        ...
        supported = result.returncode == 0 and encoder_name in output
    except Exception:
        supported = False
    _FFMPEG_ENCODER_SUPPORT_CACHE[cache_key] = supported
    return supported


def select_h264_encoder(preferred: str | None = None, ffmpeg: str | None = None) -> str:
    configured = str(preferred or os.environ.get("KEY_ACTION_FFMPEG_ENCODER") or "auto").strip().lower()
    if configured in {"", "auto"}:
        return "h264_nvenc" if ffmpeg_supports_encoder("h264_nvenc", ffmpeg=ffmpeg) else "libx264"
...
def extract_clip_ffmpeg(
    ...
    *,
    ...
    if copy_first and not reencode_requested:
        copy_cmd = [
            ffmpeg,
            "-y",
            "-ss",
    ...
    def _reencode(codec: str) -> subprocess.CompletedProcess[str]:
        cmd = [
            ffmpeg,
            "-y",
            "-ss",
        ...
        if max_width and int(max_width) > 0:
            cmd += ["-vf", f"scale=w='min({int(max_width)},iw)':h=-2"]
        if codec == "h264_nvenc":
            cmd += [
                "-preset",
                preset or "p4",
                ...
                "vbr",
                "-cq",
                str(int(crf if crf is not None else 28)),
                "-b:v",
                video_bitrate or "3M",
            ]
        else:
    ...
    codec = select_h264_encoder(video_codec or "libx264", ffmpeg=ffmpeg)
    result = _reencode(codec)
    if result.returncode != 0 and codec == "h264_nvenc":
        result = _reencode("libx264")
...
def collect_yolo_interactions_for_segment(
    ...
    for row in yolo_frame_rows:
        ...
        if local_time_sec is None:
            continue
        global_time = _row_global_time(row) or _video_sec_to_global_time(source, local_time_sec)
...
def extract_multiview_clips(
    ...
    dry_run: bool = False,
    fast_locate_include_first_person: bool | None = None,
) -> KeyActionSegment:
    ...
    third_start = _global_time_to_video_sec(third, segment.global_start_time)
    third_end = _global_time_to_video_sec(third, segment.global_end_time)
    fast_locate = _fast_locate_only()
    third_clip = Path(third.path) if fast_locate else segment_clip_dir / "third_person.mp4"
    clip_kwargs = _preview_clip_kwargs()
    if fast_locate:
        third_keyframes: list[Path] = []
    else:
        extract_clip_ffmpeg(third.path, third_start, third_end, third_clip, dry_run=dry_run, **clip_kwargs)
    ...
    if fast_locate:
        asset_bindings[0]["asset_status"] = "deferred"
        asset_bindings[0]["deferred_reason"] = "fast_locate_only"

    first_ref = None
    include_fast_first = (
        _fast_locate_include_first_person()
        if fast_locate_include_first_person is None
        else bool(fast_locate_include_first_person)
    )
    include_first = manifest.videos.first_person is not None and (not fast_locate or include_fast_first)
    if include_first:
        first = manifest.videos.first_person
        first_start = _global_time_to_video_sec(first, segment.global_start_time)
        first_end = _global_time_to_video_sec(first, segment.global_end_time)
        first_clip = Path(first.path) if fast_locate else segment_clip_dir / "first_person.mp4"
        if fast_locate:
            first_keyframes: list[Path] = []
        else:
            extract_clip_ffmpeg(first.path, first_start, first_end, first_clip, dry_run=dry_run, **clip_kwargs)
        ...
        if fast_locate:
            asset_bindings[-1]["asset_status"] = "deferred"
            asset_bindings[-1]["deferred_reason"] = "fast_locate_only"
    ...
    if fast_locate and _fast_locate_segment_interactions_enabled():
        yolo_interactions = collect_yolo_interactions_for_segment(segment, yolo_frame_rows, view_refs, view_sources)
        interaction_events = _select_interaction_events(segment.segment_id, yolo_interactions)
        interaction_keyframes = []
    elif fast_locate and dry_run and yolo_frame_rows:
