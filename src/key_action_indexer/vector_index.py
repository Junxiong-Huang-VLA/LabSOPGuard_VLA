...
class VectorIndex:
    ...
    def query(self, query_text: str, top_k: int = 5, filters: dict[str, Any] | None = None, fusion_weights: dict[str, float] | None = None) -> list[dict[str, Any]]:
        ...
        if fusion_weights is not None:
            results = rerank_results(results, query_text=query_text, fusion_weights=fusion_weights, dedup_same_segment=True, max_results=top_k)
...
def rerank_results(
    ...
    *,
    ...
    for result in results:
        text_score = float(result.get("score", result.get("similarity", 0.0)) or 0.0)

        time_score = 0.5
        duration = float(result.get("duration_sec", 0.0) or 0.0)
        if duration > 0:
            time_score = min(1.0, 0.5 + 0.5 * min(1.0, 30.0 / max(duration, 1.0)))

        has_first = bool(result.get("first_person_clip"))
        has_third = bool(result.get("third_person_clip"))
        view_score = 1.0 if (has_first and has_third) else 0.6

        evidence = result.get("evidence") if isinstance(result.get("evidence"), dict) else {}
        evidence_level = str(evidence.get("evidence_level") or result.get("evidence_level") or "")
        evidence_score = {"strong": 1.0, "moderate": 0.7, "weak": 0.4}.get(evidence_level, 0.3)
        quality = str(result.get("quality") or "")
        if quality == "high":
            evidence_score = max(evidence_score, 0.9)
        elif quality == "medium":
            evidence_score = max(evidence_score, 0.6)

        fusion_score = (
            weights.get("text_similarity", 0.5) * text_score
            + weights.get("time_proximity", 0.2) * time_score
            + weights.get("dual_view_consistency", 0.15) * view_score
            + weights.get("evidence_strength", 0.15) * evidence_score
        )

        boost_factors = {
            "text_similarity": round(text_score, 4),
            "time_proximity": round(time_score, 4),
            "dual_view_consistency": round(view_score, 4),
            "evidence_strength": round(evidence_score, 4),
            "fusion_score": round(fusion_score, 4),
        }

        reranked = dict(result)
        reranked["retrieval_boost_factors"] = boost_factors
        reranked["fusion_score"] = round(fusion_score, 6)
        scored.append((fusion_score, reranked))

    scored.sort(key=lambda item: item[0], reverse=True)
    reranked_results = [item for _, item in scored]

    if dedup_same_segment:
        seen_segments: set[str] = set()
        deduped: list[dict[str, Any]] = []
        for item in reranked_results:
            segment_id = str(item.get("segment_id") or "")
            micro_id = str(item.get("micro_segment_id") or "")
            key = micro_id or segment_id
            if not key or key not in seen_segments:
                if key:
