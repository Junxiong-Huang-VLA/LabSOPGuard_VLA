...
from .clip_extractor import select_h264_encoder
...
_LABEL_SYNONYMS.update(
    {
        "reagent_bottle_open": ["reagent_bottle_open", "open reagent bottle", "opened reagent bottle"],
        "bottle_cap": ["bottle_cap", "bottle cap", "cap"],
        "magnetic_stirrer": ["magnetic_stirrer", "magnetic stirrer"],
        "magnetic_stir_bar": ["magnetic_stir_bar", "magnetic stir bar", "stir bar", "磁力搅拌子", "搅拌子", "磁子"],
        "tube_rack": ["tube_rack", "tube rack"],
        "ppe_storage": ["ppe_storage", "PPE storage"],
    }
)
...
DEFAULT_CLASS_THRESHOLDS.update(
    {
        "reagent_bottle_open": 0.45,
        "bottle_cap": 0.45,
        "magnetic_stirrer": 0.45,
        "magnetic_stir_bar": 0.45,
        "tube_rack": 0.45,
        "ppe_storage": 0.35,
    }
)
...
def _detect_image(
    ...
    results = model.predict(
        ...
        conf=_effective_model_conf(conf, class_thresholds),
        iou=iou,
...
def _transcode_to_browser_h264(path: Path) -> bool:
    ...
    encoder = select_h264_encoder(
        os.environ.get("KEY_ACTION_ANNOTATED_VIDEO_CODEC") or os.environ.get("KEY_ACTION_FFMPEG_ENCODER") or "auto",
        ffmpeg=ffmpeg_exe,
    )
    crf = str(int(float(os.environ.get("KEY_ACTION_ANNOTATED_VIDEO_CRF", "26"))))
    bitrate = os.environ.get("KEY_ACTION_ANNOTATED_VIDEO_BITRATE")
    max_width = int(float(os.environ.get("KEY_ACTION_ANNOTATED_VIDEO_MAX_WIDTH", "0")))

    def _command(codec: str) -> list[str]:
        cmd = [ffmpeg_exe, "-y", "-i", str(path), "-c:v", codec]
        if max_width > 0:
            cmd += ["-vf", f"scale=w='min({max_width},iw)':h=-2"]
        if codec == "h264_nvenc":
            cmd += [
                "-preset",
                os.environ.get("KEY_ACTION_ANNOTATED_VIDEO_PRESET") or "p4",
                "-rc",
                "vbr",
                "-cq",
                crf,
            ...
            cmd += [
                "-preset",
                os.environ.get("KEY_ACTION_ANNOTATED_VIDEO_PRESET") or "veryfast",
                "-crf",
                crf,
            ]
            if bitrate:
    ...
    try:
        result = subprocess.run(_command(encoder), stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, timeout=600)
        if result.returncode != 0 and encoder == "h264_nvenc":
            result = subprocess.run(_command("libx264"), stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, timeout=600)
...
def _load_or_extract_focus_summary(session_root: Path) -> dict[str, Any]:
    summary_path = session_root / "metadata" / "experiment_focus_clips.json"
    existing = _read_json(summary_path)
    clips_by_view = existing.get("clips_by_view") if isinstance(existing.get("clips_by_view"), dict) else {}
    if clips_by_view:
        has_existing_clip = False
        for ref in clips_by_view.values():
            if not isinstance(ref, dict):
                continue
            clip_path = Path(str(ref.get("clip_path") or ""))
            if clip_path.exists() and clip_path.stat().st_size > 0:
                has_existing_clip = True
                break
        if has_existing_clip:
            return existing
    return extract_experiment_focus_clips(session_root, dry_run=False, force_extract=True)


def _candidate_yolo_frame_row_paths(session_root: Path) -> list[Path]:
    return [
        session_root / "cv_outputs" / "yolo_frame_rows.jsonl",
        session_root / "metadata" / "yolo_frame_rows.jsonl",
        session_root / "cv_outputs" / "yolo_micro_frame_rows.jsonl",
    ]


def _load_yolo_frame_rows_for_overlay(session_root: Path, row_paths: list[str | Path] | tuple[str | Path, ...] | None = None) -> tuple[Path | None, list[dict[str, Any]]]:
    candidates = [Path(path) for path in row_paths] if row_paths else _candidate_yolo_frame_row_paths(session_root)
...
def _row_view(row: dict[str, Any]) -> str:
    return str(row.get("view") or row.get("source_view") or row.get("requested_view") or "").strip()


def _row_local_time(row: dict[str, Any]) -> float | None:
    return _safe_float(row.get("local_time_sec"), _safe_float(row.get("time_sec")))


def _normalise_overlay_detections(
    row: dict[str, Any],
    *,
    target_width: int,
    target_height: int,
    ...
    if not isinstance(detections, list):
        return []
    source_width = _safe_float(row.get("frame_width"), float(target_width)) or float(target_width)
    source_height = _safe_float(row.get("frame_height"), float(target_height)) or float(target_height)
    ...
    for detection in detections:
        ...
        next_detection["confidence"] = float(
            _safe_float(detection.get("confidence"), _safe_float(detection.get("score"), 1.0))
            or 1.0
        )
        next_detection["bbox"] = [
            max(0.0, min(float(target_width - 1), x1 * scale_x)),
            max(0.0, min(float(target_height - 1), y1 * scale_y)),
            max(0.0, min(float(target_width - 1), x2 * scale_x)),
            max(0.0, min(float(target_height - 1), y2 * scale_y)),
        ]
        normalized.append(next_detection)
    return normalized


def annotate_clip_with_detection_rows(
    input_clip_path: str | Path,
    output_clip_path: str | Path,
    rows: list[dict[str, Any]],
    *,
    local_start_sec: float = 0.0,
    max_hold_sec: float = 2.5,
    source_view: str | None = None,
) -> dict[str, Any]:
    import cv2
    ...
    target.parent.mkdir(parents=True, exist_ok=True)
    timed_rows = sorted(
        ((float(time_value), row) for row in rows if (time_value := _row_local_time(row)) is not None),
        key=lambda item: item[0],
    )
    cap = cv2.VideoCapture(str(source))
    if not cap.isOpened():
        raise RuntimeError(f"Cannot open clip for detection-row annotation: {source}")
    try:
        fps = float(cap.get(cv2.CAP_PROP_FPS) or 15.0)
        width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH) or 0)
        height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT) or 0)
        ...
        while True:
            ...
            if not ok:
                break
            source_time_sec = float(local_start_sec) + (frame_idx / fps if fps > 0 else 0.0)
            while row_idx + 1 < len(timed_rows) and timed_rows[row_idx + 1][0] <= source_time_sec:
                row_idx += 1
            candidate_indexes = [row_idx]
            if row_idx + 1 < len(timed_rows):
                candidate_indexes.append(row_idx + 1)
            best_index = min(candidate_indexes, key=lambda idx: abs(timed_rows[idx][0] - source_time_sec)) if timed_rows else None
            detections: list[dict[str, Any]] = []
            if best_index is not None and abs(timed_rows[best_index][0] - source_time_sec) <= max_hold_sec:
                used_row_indexes.add(best_index)
                detections = _normalise_overlay_detections(timed_rows[best_index][1], target_width=width, target_height=height)
            for detection in detections:
                label_counts[str(detection.get("label"))] += 1
            writer.write(_draw_detections_on_frame(frame, detections))
            frame_idx += 1
        writer.release()
    finally:
        cap.release()

    for _time_sec, row in timed_rows:
        for detection in row.get("detections") or []:
            if isinstance(detection, dict):
                raw_label_counts[str(detection.get("label") or detection.get("class_name") or detection.get("raw_label") or "object")] += 1
    browser_h264 = _transcode_to_browser_h264(target)
    return {
        "input_clip_path": str(source),
        "annotated_clip_path": str(target),
        ...
        "frame_count": frame_idx,
        "source_frame_count": total_frames,
        "detection_rows": len(timed_rows),
        "used_detection_rows": len(used_row_indexes),
        "label_counts": dict(label_counts),
        "raw_label_counts": dict(raw_label_counts),
        "detections": int(sum(label_counts.values())),
        "raw_detections": int(sum(raw_label_counts.values())),
...
def render_experiment_focus_overlay_from_yolo_rows(
    session_dir: str | Path,
    *,
    preferred_view: str = "first_person",
    views: list[str] | tuple[str, ...] | None = None,
    row_paths: list[str | Path] | tuple[str | Path, ...] | None = None,
    ...
    hold_sec = float(
        ...
        else os.environ.get("KEY_ACTION_YOLO_ROW_OVERLAY_HOLD_SEC", "2.5")
    )
    focus_summary = _load_or_extract_focus_summary(session_root)
    clips_by_view = focus_summary.get("clips_by_view") if isinstance(focus_summary.get("clips_by_view"), dict) else {}
    row_source_path, detection_rows = _load_yolo_frame_rows_for_overlay(session_root, row_paths=row_paths)
    if not detection_rows:
        summary = {
            "available": False,
            "status": "failed",
            "error": "No yolo_frame_rows.jsonl rows found for overlay rendering",
            "clips": 0,
            "detections": 0,
    ...
    if not force and existing_summary.get("available") and existing_summary.get("render_source") == "yolo_frame_rows":
        existing_rows = read_jsonl(output_path) if output_path.exists() else []
        existing_paths = [Path(str(row.get("annotated_clip_path") or "")) for row in existing_rows if isinstance(row, dict)]
        if existing_paths and all(path.exists() and path.stat().st_size > 0 for path in existing_paths):
            existing_summary["cache_status"] = "hit"
            return existing_summary

    rows_by_view: dict[str, list[dict[str, Any]]] = defaultdict(list)
    ...
    raw_total_labels: Counter[str] = Counter()
    label_counts_by_view: dict[str, Counter[str]] = defaultdict(Counter)
    clip_counts_by_view: Counter[str] = Counter()
    errors: list[str] = []
    for view in requested_views:
        view_ref = clips_by_view.get(view)
        if not isinstance(view_ref, dict):
            errors.append(f"{view}: experiment focus clip metadata not found")
            continue
        clip_path = Path(str(view_ref.get("clip_path") or ""))
        if not clip_path.exists():
            errors.append(f"{view}: experiment focus clip not found: {clip_path}")
            continue
        local_start = _safe_float(view_ref.get("local_start_sec"), 0.0) or 0.0
        local_end = _safe_float(view_ref.get("local_end_sec"), local_start) or local_start
        view_rows = [
            row
            for row in rows_by_view.get(view, [])
            if (row_time := _row_local_time(row)) is not None
            and float(row_time) >= local_start - hold_sec
            and float(row_time) <= local_end + hold_sec
        ]
        output_clip = session_root / "clips" / "experiment_focus" / f"{view}_yolo_annotated.mp4"
        try:
            clip_result = annotate_clip_with_detection_rows(
                clip_path,
                output_clip,
                view_rows,
        ...
        raw_total_labels.update(clip_result.get("raw_label_counts", {}))
        ...
        rows.append(
            {
                ...
                "view": view,
                "requested_view": view,
                "model_path": ((view_rows[0].get("model_ref") or {}).get("path") if view_rows and isinstance(view_rows[0].get("model_ref"), dict) else None),
                "time_start_sec": view_ref.get("time_start_sec"),
                "time_end_sec": view_ref.get("time_end_sec"),
                "local_start_sec": view_ref.get("local_start_sec"),
                "local_end_sec": view_ref.get("local_end_sec"),
                "global_start_time": view_ref.get("global_start_time"),
                "global_end_time": view_ref.get("global_end_time"),
                "render_source": "yolo_frame_rows",
                "row_source_path": str(row_source_path) if row_source_path else None,
                **clip_result,
            }
        )
    ...
    summary = {
        ...
        "raw_detections": int(sum(raw_total_labels.values())),
        "label_counts": dict(total_labels),
        "raw_label_counts": dict(raw_total_labels),
...
def render_segment_overlays_from_yolo_rows(
    session_dir: str | Path,
    *,
    preferred_view: str = "first_person",
    views: list[str] | tuple[str, ...] | None = None,
    ...
    if not detection_rows:
        summary = {
            "available": False,
            "status": "failed",
            "error": "No yolo_frame_rows.jsonl rows found for segment overlay rendering",
            "clips": 0,
            "detections": 0,
            "views": requested_views,
            "render_source": "yolo_frame_rows",
    ...
    def resolve_clip_path(raw: Any) -> Path:
        clip_path = Path(str(raw or ""))
        if clip_path.is_absolute():
            return clip_path
        cwd_candidate = Path.cwd() / clip_path
        if cwd_candidate.exists():
            return cwd_candidate
        return session_root / clip_path
    ...
    for segment in segments:
        ...
        for requested_view in requested_views:
            ...
            if not isinstance(view_ref, dict):
                errors.append(f"{segment_id}/{requested_view}: clip metadata not found")
                continue
            clip_path = resolve_clip_path(view_ref.get("clip_path"))
            if not clip_path.exists():
                errors.append(f"{segment_id}/{requested_view}: clip not found: {clip_path}")
                continue
            local_start = _safe_float(view_ref.get("local_start_sec"), _safe_float(segment.get("local_start_sec"), 0.0)) or 0.0
            local_end = _safe_float(view_ref.get("local_end_sec"), _safe_float(segment.get("local_end_sec"), local_start)) or local_start
            view_rows = [
                row
                for row in rows_by_view.get(requested_view, [])
                if (row_time := _row_local_time(row)) is not None
                and float(row_time) >= local_start - hold_sec
                and float(row_time) <= local_end + hold_sec
            ]
            output_clip = clip_path.with_name(f"{requested_view}_yolo_annotated.mp4")
            try:
                clip_result = annotate_clip_with_detection_rows(
                    clip_path,
                    output_clip,
                ...
                errors.append(f"{segment_id}/{requested_view}: {exc}")
                continue
            total_labels.update(clip_result.get("label_counts", {}))
            raw_total_labels.update(clip_result.get("raw_label_counts", {}))
            ...
            rows.append(
                {
                    ...
                    "model_path": (
                        (view_rows[0].get("model_ref") or {}).get("path")
                        if view_rows and isinstance(view_rows[0].get("model_ref"), dict)
                        else None
                    ),
                    "local_start_sec": local_start,
...
def run_yolo_on_experiment_focus_clips(
    ...
    *,
    ...
    try:
        focus_summary = extract_experiment_focus_clips(session_root, dry_run=False, force_extract=True)
    except Exception as exc:
        summary = {
            "available": False,
            "error": str(exc),
...
def _tools_objects_from_labels(label_counts: Counter[str]) -> tuple[list[str], list[str]]:
    tool_labels = {"balance", "pipette", "pipette_tip", "spatula", "magnetic_stirrer", "magnetic_stir_bar"}
    object_labels = {
        "sample_bottle_blue",
        "sample_bottle",
        "reagent_bottle",
