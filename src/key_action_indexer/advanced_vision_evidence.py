...
from .scene_stabilizer import estimate_scene_motion_from_points
...
def build_advanced_vision_evidence(session_dir: str | Path, output_path: str | Path | None = None) -> dict[str, Any]:
    ...
    gate_decisions = [
        ...
        for row in evidence
        if isinstance(row.get("physical_event_gate"), Mapping)
    ]
    gate_path = metadata / "physical_event_gate_decisions.jsonl"
    rejected_path = metadata / "rejected_physical_event_candidates.jsonl"
    existing_gate_decisions = _read_jsonl_if_exists(gate_path)
    if gate_decisions or existing_gate_decisions:
        write_jsonl(gate_path, [_stable_gate(row) for row in [*existing_gate_decisions, *gate_decisions]])
    existing_rejected = _read_jsonl_if_exists(rejected_path)
    rejected_gate_rows = []
    for row in evidence:
        gate = row.get("physical_event_gate") if isinstance(row.get("physical_event_gate"), Mapping) else {}
        if gate.get("status") == "rejected":
            rejected_gate_rows.append(
                {
                    "candidate_id": row.get("evidence_id"),
                    "event_type": gate.get("event_type") or row.get("evidence_type"),
                    "status": "rejected",
                    "time_start": row.get("global_start_time"),
                    "time_end": row.get("global_end_time"),
                    "source_view": _as_dict(row.get("multiview_consistency")).get("views"),
                    "actor_track_id": None,
                    "object_track_ids": [row.get("metrics", {}).get("track_id")] if isinstance(row.get("metrics"), Mapping) and row.get("metrics", {}).get("track_id") else [],
    ...
    if rejected_gate_rows or existing_rejected:
        write_jsonl(rejected_path, [_stable_rejected(row) for row in [*existing_rejected, *rejected_gate_rows]])
    ...
    summary = {
        ...
        "physical_event_gate_summary": summarize_gate_decisions([*existing_gate_decisions, *gate_decisions])
        if (gate_decisions or existing_gate_decisions)
        else {},
        "input_counts": {
...
def _hand_object_contact_evidence(
    ...
    gate = gate_hand_object_contact(
        event_candidate={
            "candidate_id": str(micro.get("micro_segment_id") or ""),
            "event_type": "hand_object_interaction",
            "object_labels": [primary] if primary else [],
            "time_start": micro.get("global_start_time"),
            "time_end": micro.get("global_end_time"),
        },
        object_track={"track_id": str(interaction.get("primary_object_track_id") or primary)} if primary else None,
        frame_evidence_list=row_matches,
        external_observation={
            "has_hand": bool(row_matches or scores),
            "has_object": bool(primary),
            "contact_frames": frame_count,
            "continuous_contact_frames": frame_count,
            "overlap_frames": len([value for value in overlaps if value > 0.0]),
            "max_iou": max(overlaps or [0.0]),
            "min_distance_px": min(distances) if distances else None,
            "near_only": bool(distances and not any(value > 0.0 for value in overlaps)),
            "contact_type": interaction_type if any(token in interaction_type for token in ("grasp", "hold", "press", "touch", "contact")) else None,
        },
    )
    if gate.get("status") == "rejected" and (primary or interaction_type):
        gate = {
            **gate,
            "status": "candidate",
            "confidence": max(0.3, _float(gate.get("confidence"), 0.0)),
            "reject_reasons": _dedupe(
                [
                    *(gate.get("reject_reasons") or []),
                    "metadata_only_contact_unconfirmed",
                ]
            ),
            "limitations": _dedupe(
                [
                    *(gate.get("limitations") or []),
                    "hand-object contact metadata lacks hard YOLO overlap evidence",
                ]
            ),
        }
        gate["hard_gate"] = {**dict(gate.get("hard_gate") or {}), "passed": False}
    gate_status = str(gate.get("status") or "")
    visual_level = "hand_object_contact_confirmed" if gate_status == "confirmed" else "hand_object_contact_candidate"
    limitations = []
    if frame_count < 2:
...
def _trajectory_evidence(micro: Mapping[str, Any], yolo_rows: list[Mapping[str, Any]], assets: list[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    for label in candidate_labels:
        ...
        for item in label_tracks:
            ...
            event = _trajectory_event(micro, label, item, track, assets, label_context, scene_rows=yolo_rows)
            if event:
                events.append(event)
                break
    return events
...
def _trajectory_event(
    ...
    *,
    scene_rows: list[Mapping[str, Any]] | None = None,
) -> dict[str, Any] | None:
    first = track[0]
    last = track[-1]
    ...
    low_motion_prefilter = normalized < 0.25 and path_length / scale < 0.5
    ...
    track_type = "tracker_track" if _explicit_track_identity(track_item) else "inferred_track"
    track_ev = track_evidence_from_points(
        track_id=str(track_item.get("track_id") or ""),
        object_label=label,
        points=track,
        track_type=track_type,
        source_view=str(first.get("view") or ""),
        identity_confidence=identity_confidence,
        id_switch_risk=1.0 - identity_confidence,
        can_confirm_motion=track_type != "label_level_pseudo_track",
        limitations=limitations,
    )
    interaction = _as_dict(micro.get("interaction"))
    hand_contact = {
        "status": "confirmed"
        if _float(interaction.get("max_interaction_score"), 0.0) >= 0.65
        else "candidate"
    }
    gate = gate_object_move(
        event_candidate={
            "candidate_id": f"{micro.get('micro_segment_id')}:{label}:trajectory",
            "event_type": "object_move",
            "time_start": micro.get("start_sec"),
            "time_end": micro.get("end_sec"),
            "source_view": first.get("view"),
            "object_labels": [label],
        },
        track=track_ev,
        scene_motion=estimate_scene_motion_from_points(scene_rows),
        hand_contact=hand_contact,
    )
    gate_status = str(gate.get("status") or "")
    if gate_status == "confirmed":
        visual_level = "trajectory_confirmed"
    elif gate_status == "candidate":
        visual_level = "trajectory_candidate_gate_review"
    else:
        visual_level = "trajectory_rejected_by_gate"
    ...
    event = _event(
        ...
        evidence_reasons=[
            ...
            f"physical_event_gate_status={gate_status}",
            f"low_motion_prefilter={str(low_motion_prefilter).lower()}",
        ],
        limitations=[*limitations, *(gate.get("limitations") or [])],
        metrics={
            "track_id": track_item.get("track_id"),
            "track_source": source,
            "track_type": track_type,
            "can_confirm_motion": _as_dict(gate.get("evidence")).get("can_confirm_motion"),
            "identity_confidence": identity_confidence,
            "id_switch_risk": _as_dict(gate.get("evidence")).get("id_switch_risk"),
            "identity_source": track_item.get("identity_source"),
            "identity_status": "stable" if gate_status == "confirmed" else "same_class_or_gate_risk",
...
def _image_pair_evidence(
    ...
    return [_gate_image_pair_event(event, primary) for event in events]
...
def _panel_evidence(session: Path, micro: Mapping[str, Any], assets: list[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    if ocr_text:
        ...
        gate = gate_panel_operation(
            event_candidate={
                "candidate_id": event.get("evidence_id"),
                "event_type": "panel_operation",
                "time_start": event.get("global_start_time"),
                "time_end": event.get("global_end_time"),
                "object_labels": [primary] if primary else [],
            },
            device_track={"track_id": primary} if primary else None,
            display_state={
                "display_text_after": ocr_text,
...
def _model_observation_evidence(rows: list[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        if _confirmed_or_measured_level(visual_level, confirmation_level) and not _model_observation_has_hard_evidence(row, evidence_type):
            visual_level = _downgrade_model_observation_visual_level(visual_level)
            confirmation_level = "candidate"
            limitations = _dedupe(
                [
                    *limitations,
                    "model observation downgraded; missing physical_event_gate hard evidence",
                ]
            )
        reasons = _dedupe(
            [
...
def _model_observation_has_hard_evidence(row: Mapping[str, Any], evidence_type: str) -> bool:
    gate = row.get("physical_event_gate")
    if isinstance(gate, Mapping):
        return gate.get("status") == "confirmed" and isinstance(gate.get("hard_gate"), Mapping) and gate["hard_gate"].get("passed") is True
    metrics = _as_dict(row.get("metrics"))
    measurement = _as_dict(row.get("measurement"))
    payload = {**metrics, **measurement, **_as_dict(row.get("payload"))}
    if evidence_type == "object_trajectory_movement":
        return bool(payload.get("stable_instance_track") and payload.get("stabilized_displacement_px"))
    if evidence_type in {"liquid_level_change", "liquid_flow_candidate_visual"}:
        return any(payload.get(key) for key in ("liquid_level_delta", "droplet_detected", "stream_detected", "liquid_region_changed"))
    if evidence_type == "equipment_control_change":
        return any(payload.get(key) for key in ("hand_control_roi_contact", "display_changed", "button_state_changed", "knob_angle_delta", "switch_state_changed"))
    if evidence_type in {"container_open_close", "container_color_change"}:
        return bool(payload.get("same_container_instance") and payload.get("changed_fields"))
    return False


def _downgrade_model_observation_visual_level(visual_level: str) -> str:
    text = str(visual_level or "model_observation")
    for token in ("_confirmed", "_measured"):
        if token in text:
            return text.replace(token, "_candidate")
    if text.endswith("confirmed") or text.endswith("measured"):
        return f"{text.rsplit('_', 1)[0]}_candidate"
    return f"{text}_candidate"
...
def _with_gate(event: dict[str, Any], gate: Mapping[str, Any]) -> dict[str, Any]:
    status = str(gate.get("status") or "uncertain")
    event["status"] = status
    event["hard_gate"] = gate.get("hard_gate") or {}
    event["evidence_detail"] = gate.get("evidence") or {}
    event["reject_reasons"] = list(gate.get("reject_reasons") or [])
    event["physical_event_gate"] = dict(gate)
    event["limitations"] = _dedupe([*(event.get("limitations") or []), *(gate.get("limitations") or [])])
    if status != "confirmed":
        event["confirmation_level"] = status
        event["requires_human_confirmation"] = True
        visual_level = str(event.get("visual_confirmation_level") or "")
        if "by_gate" not in visual_level:
            event["visual_confirmation_level"] = f"{visual_level}_{status}_by_gate" if visual_level else f"{status}_by_gate"
    else:
        event["requires_human_confirmation"] = bool(float(event.get("confidence") or 0.0) < 0.65)
    return event


def _gate_image_pair_event(event: dict[str, Any], primary: str) -> dict[str, Any]:
    ...
    if evidence_type in {"liquid_level_change", "liquid_flow_candidate_visual"}:
        liquid_obs = {
            "has_liquid_region": bool(metrics.get("image_metrics") or metrics.get("keyframe_visual_indicators")),
            "liquid_level_delta": metrics.get("liquid_level_delta"),
            "liquid_region_changed": bool(metrics.get("liquid_level_delta")),
            "visual_change_score": metrics.get("liquid_level_delta") or 0.0,
            "stream_detected": False,
            "droplet_detected": False,
        }
        gate = gate_liquid_transfer(event_candidate=candidate, liquid_observation=liquid_obs)
        return _with_gate(event, gate)
    if evidence_type == "container_color_change":
        color = _as_dict(metrics.get("color_change_indicator"))
        gate = gate_container_state_change(
            event_candidate=candidate,
            container_track={"track_id": primary} if primary else None,
            pre_state={"content_color": color.get("first_color_family")},
            post_state={"content_color": color.get("last_color_family")},
            frame_pair_evidence={
                "changed_fields": ["content_color"] if color.get("first_color_family") != color.get("last_color_family") else [],
                "lighting_change_risk": float(color.get("brightness_delta") or 0.0) > 0.25,
            },
        )
        return _with_gate(event, gate)
    if evidence_type == "container_open_close":
        indicators = _as_dict(metrics.get("container_state_indicators"))
        cap_tokens = metrics.get("cap_lid_tokens") or []
        gate = gate_container_state_change(
            event_candidate=candidate,
            container_track={"track_id": primary} if primary else None,
            pre_state=None,
            ...
            frame_pair_evidence={
                "changed_fields": [],
                "cap_lid_tokens": cap_tokens,
                "state_signal": indicators.get("state_signal"),
            },
        )
        return _with_gate(event, gate)
    return event
