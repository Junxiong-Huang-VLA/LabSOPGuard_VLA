...
def _track_row(
    *,
    ...
    moving = displacement >= min_motion_px or path_length >= min_motion_px
    ...
    if not moving:
        limitations.append("tracked object is static in available sampled frames")
    reasons = [
    ...
    return {
        ...
        "object_label": label,
        "track_id": track_id,
        "state": "moving" if moving else "tracked_static",
        "motion_state": "moving" if moving else "tracked_static",
        "event_type": "object_movement_measured" if moving else "object_track_observed",
        "confirmation_level": "measured" if moving else "confirmed",
        ...
        "identity_confidence": 0.72 if moving else 0.62,
