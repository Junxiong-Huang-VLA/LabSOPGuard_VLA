...
def _validate_adapter(
    ...
    *,
    ...
    if len(views) == 1 and _warn_single_view_adapter(adapter_name):
...
def _semantic_support_issues(
    ...
    *,
    ...
    if not relation.get("time_overlap_ok") and not _is_unlinked_candidate_context_signal(row):
        issues.append(
            _issue(
                "warning",
...
def _row_relation(row: Mapping[str, Any], context: Mapping[str, Any], *, time_tolerance_sec: float = 0.0) -> dict[str, Any]:
    ...
    time_overlap_ok = bool(
        overlap > 0.0
        or _point_event_within_interval((start, end), related_window)
        or gap == 0.0
...
def _point_event_within_interval(left: tuple[float | None, float | None], right: tuple[float | None, float | None]) -> bool:
    left_start, left_end = left
    right_start, right_end = right
    if left_start is None or left_end is None or right_start is None or right_end is None:
        return False
    if abs(left_end - left_start) <= 1e-6:
        return right_start <= left_start <= right_end
    if abs(right_end - right_start) <= 1e-6:
...
def _is_unlinked_candidate_context_signal(row: Mapping[str, Any]) -> bool:
    if str(row.get("segment_id") or "").strip() or str(row.get("micro_segment_id") or "").strip():
        return False
    level = _norm(row.get("confirmation_level") or row.get("status") or row.get("state") or "")
    event_type = _norm(row.get("event_type") or row.get("observation_type") or "")
    if "candidate" in level or "candidate" in event_type:
        return True
    return False


def _warn_single_view_adapter(adapter_name: str) -> bool:
    return adapter_name in {"object_tracks", "panel_ocr"}
