...
def _action_from_primary(primary: Any) -> str:
    ...
    if label in {"pipette", "pipette_tip"}:
        return "pipetting"
    if label in {"balance", "scale", "panel", "display"}:
        return "equipment_panel_operation"
    if label in {"paper", "weighing_paper"}:
        return "weighing_paper_operation"
    if label == "spatula":
        return "spatula_interaction"
    if label in {"reagent_bottle", "reagent_bottle_open", "bottle_cap", "sample_bottle", "sample_bottle_blue", "bottle"}:
        return "reagent_bottle_interaction"
    if label == "magnetic_stir_bar":
