...
class ExperimentService:
    ...
    def _infer_view_type(camera_id: Any, source_type: Any, descriptor: Dict[str, Any]) -> str:
        ...
        if cam in {"ptz", "ptz_tracker", "pan_tilt"} or src == "ptz":
            return "overview"
        if cam.startswith("wireless") or src in {"rtsp", "http", "rtmp", "udp"}:
            return "third_person"
        if cam.startswith("usb") or src == "usb":
            return "first_person"
        return "first_person"
    ...
    def _infer_source_group(camera_id: Any, source_type: Any, view_type: str, descriptor: Dict[str, Any]) -> str:
        ...
        cam = str(camera_id or "").strip().lower()
        src = str(source_type or "").strip().lower()
        if view_type == "overview" or cam in {"ptz", "ptz_tracker", "pan_tilt"} or src == "ptz":
            return "ptz"
        if cam.startswith("wireless") or cam.startswith("usb") or src in {"rtsp", "http", "rtmp", "udp", "usb"}:
                ...
                if getattr(item, "change_score", 0.0) >= object_motion_threshold:
                    events.append(
                        PhysicalEvent(
                            ...
                            duration_sec=self._frame_extractor.sample_interval_sec,
                            location=location,
                            confidence=max(conf, min(0.98, 0.45 + getattr(item, "change_score", 0.0))),
                            provenance=ProvenanceInfo(
                                source="video",
                                source_id=f"frame_{frame['frame_id']}",
                                confidence=conf,
                                is_inferred=True,
                                inference_method="visual_motion_proxy",
                                model_used=model,
                            ),
                            metadata={
                                **base_metadata,
    ...
    def save_outputs(self, output_dir: str = "outputs/experiments") -> Dict[str, str]:
        ...
        # 物理事件
        pe_path = out_dir / "physical_events.json"
        pe_data = [pe.to_dict() for pe in self._experiment.physical_events]
        pe_path.write_text(
