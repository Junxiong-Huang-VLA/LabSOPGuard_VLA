...
def _path_mtime(path: Path, default: Optional[float] = None) -> Optional[float]:
    try:
...
def _semantic_material_terms(row: Dict[str, Any]) -> List[str]:
    ...
    terms.extend(_flatten_search_terms(row.get("secondary_objects")))
    terms.extend(_flatten_search_terms(row.get("secondary_actions")))
...
def _formal_material_reference_items(exp_dir: Path, experiment_id: str) -> Dict[str, Any]:
    ...
    if ref_root is None:
        return {"items": [], "source": None, "source_mtime": _path_mtime(exp_dir)}
    ...
    for index, row in enumerate(rows, start=1):
        ...
        for secondary_object in _flatten_search_terms(row.get("secondary_objects")):
            if secondary_object not in object_labels:
                object_labels.append(secondary_object)
        actions = row.get("actions") or [row.get("action_name") or asset_kind]
        actions = [str(action) for action in actions if str(action or "").strip()]
        canonical_action = str(row.get("canonical_action_type") or "").strip()
        if canonical_action and canonical_action not in actions:
            actions.append(canonical_action)
        for secondary_action in _flatten_search_terms(row.get("secondary_actions")):
            if secondary_action not in actions:
        ...
        if not evidence_chain:
            evidence_chain = {
                "schema_version": "material_reference_trace.v1",
                "source_clip": row.get("source_clip") or row.get("source_clip_path") or row.get("source_file"),
                "camera_view": row.get("camera_view") or row.get("view"),
                "time_start": start_sec,
                "time_end": end_sec,
                "yolo_evidence_count": row.get("yolo_evidence_count"),
                "canonical_action_type": row.get("canonical_action_type"),
                "candidate_disposition": row.get("candidate_status") or row.get("disposition"),
            }
        item = enrich_material_taxonomy(
            {
                **row,
                ...
                "source_clip": row.get("source_clip") or row.get("source_clip_path") or row.get("source_file"),
                "source_clip_path": row.get("source_clip_path") or row.get("source_clip") or row.get("source_file"),
                "camera_view": row.get("camera_view") or row.get("view"),
    ...
    source_mtime = _path_mtime(index_path) if index_path.exists() else _path_mtime(ref_root)
...
def rebuild_workspace_published_materials_index(
    ...
    for exp_dir in sorted(path for path in root.iterdir() if path.is_dir()):
        ...
        source_mtime = _path_mtime(published_path) if published_path.exists() else _path_mtime(exp_dir, 0.0)
...
def _workspace_published_lifecycle_snapshot(experiments_root: Path, index_path: Path) -> Dict[str, Any]:
    ...
    for exp_dir in sorted(path for path in experiments_root.iterdir() if path.is_dir()):
        ...
        if published_path.exists():
            ...
            source_mtime = _path_mtime(published_path)
        else:
            fallback = _formal_material_reference_items(exp_dir, experiment_id)
            ...
            if fallback_items or formal_counts["material_count"] or formal_counts["report_count"]:
                ...
                source_mtime = fallback.get("source_mtime") or formal_counts.get("source_mtime") or _path_mtime(exp_dir)
    ...
    index_mtime = _path_mtime(index_path) if index_exists else None
...
def _insert_workspace_published(conn: sqlite3.Connection, item: Dict[str, Any]) -> None:
    ...
    text = " ".join(
        ...
        for part in [
            ...
            item.get("best_score"),
            item.get("source_clip"),
            item.get("camera_view"),
...
_CANONICAL_QUERY_RULES = [
    ...
    {
        "needles": ("药匙", "加样", "刮取", "固体样品", "spatula", "scoop", "solid"),
        "canonical_action_type": "hand-spatula",
        "canonical_object": "spatula",
        "sop_phase": "solid-transfer",
        "terms": ("药匙", "加样", "刮取", "固体样品", "spatula", "solid transfer"),
    },
    {
    ...
    {
        "needles": ("承接", "接样", "样品进入", "container", "hand-container", "container-handling"),
