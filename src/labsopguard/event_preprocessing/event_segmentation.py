...
class EventSegmenter:
        ...
        for index, proposal in enumerate(merged, 1):
            ...
            events.append(
                PhysicalEvent(
                    ...
                    event_status=proposal.status or ("confirmed" if proposal.review_status == "auto_confirmed" else "candidate"),
                    ...
                    hard_gate=proposal.hard_gate,
                    evidence_detail=proposal.evidence_detail,
                    reject_reasons=proposal.reject_reasons,
                    limitations=proposal.limitations,
                    qwen_audit=proposal.qwen_audit,
                )
            )
        return events
    ...
    def _merge_proposals(self, proposals: List[EventProposal]) -> List[EventProposal]:
        ...
        for proposal in ordered:
            ...
            if (
                ...
                prev.status = self._merge_status(prev.status, proposal.status)
                prev.reject_reasons = list(dict.fromkeys([*prev.reject_reasons, *proposal.reject_reasons]))
                prev.limitations = list(dict.fromkeys([*prev.limitations, *proposal.limitations]))
                if not prev.hard_gate and proposal.hard_gate:
                    prev.hard_gate = proposal.hard_gate
                prev.evidence_detail = {**(prev.evidence_detail or {}), **(proposal.evidence_detail or {})}
            else:
                merged.append(proposal)
    ...
    def _merge_status(left: str, right: str) -> str:
        order = {"rejected": 0, "uncertain": 1, "candidate": 2, "confirmed": 3}
        return left if order.get(left, 0) >= order.get(right, 0) else right
