...
class _LiveTrack:
    ...
    def to_tracklet(self) -> Tracklet:
        first_center = _center(self.bboxes[0]) if self.bboxes else (0.0, 0.0)
        last_center = _center(self.bboxes[-1]) if self.bboxes else first_center
...
class IouMultiObjectTracker:
    """Small dependency-free MOT baseline.

    This is intentionally swappable: when a trained lab-action tracker or ByteTrack/BoT-SORT
    dataset is available, keep the DetectionFrame/Tracklet contract and replace this class.
    """
    ...
    def _update(self, frame: DetectionFrame) -> None:
        ...
        for det_idx in unmatched_detections:
            ...
            track = _LiveTrack(
                ...
                last_time_sec=frame.timestamp_sec,
            )
