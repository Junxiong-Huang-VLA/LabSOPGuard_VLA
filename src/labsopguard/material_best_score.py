...
MATERIAL_BEST_SCORE_SCHEMA_VERSION = "material_best_score.v2"
MATERIAL_BEST_REASON_SCHEMA_VERSION = "material_best_reason.v1"
...
def enrich_material_best_score(item: Dict[str, Any], *, peers: Optional[Iterable[Dict[str, Any]]] = None) -> Dict[str, Any]:
    ...
    scored.update(
        {
            "best_score_schema_version": MATERIAL_BEST_SCORE_SCHEMA_VERSION,
            "best_reason_schema_version": MATERIAL_BEST_REASON_SCHEMA_VERSION,
...
def material_best_reason(item: Dict[str, Any], breakdown: Dict[str, Any]) -> str:
    ...
    view_text = ", ".join(str(view) for view in breakdown.get("views") or []) or "one evidence view"
    best_score = float(breakdown.get("best_score") or 0)
    interaction = float(breakdown.get("hand_object_interaction_score") or 0)
    reasons = [
        f"Selected as the best {label} material for {obj}: best_score {best_score:.3f} combines visual quality, YOLO evidence, hand-object interaction, view support, and SOP phase match.",
        f"The window {start:.2f}-{end:.2f}s has quality {breakdown.get('quality_score', 0):.2f}, {int(breakdown.get('yolo_evidence_count') or 0)} YOLO evidence frames, interaction strength {interaction:.2f}, and view support from {view_text}.",
        f"It maps cleanly to SOP phase {item.get('sop_phase') or '-'}, so it is a representative library asset rather than a frame where the object is merely present.",
    ]
    penalties: List[str] = []
    if float(breakdown.get("duplicate_penalty") or 0) > 0:
        penalties.append("near-duplicate penalty")
    if float(breakdown.get("bad_visibility_penalty") or 0) > 0:
        penalties.append("visibility/access penalty")
    if penalties:
        reasons.append(f"Adjusted for {' and '.join(penalties)}.")
    return " ".join(reasons)
